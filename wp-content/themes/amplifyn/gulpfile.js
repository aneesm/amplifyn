...
function scssTask(){
    return src(`${paths.src.css}style.scss`)
        ...
        .pipe(sourcemaps.write(paths.wordpress.css)) // save sourcemap
        .pipe(dest(paths.wordpress.css)); // put final CSS in patternlab public folder
}
...
function jsDevTask(){
    return src(`${paths.src.js}main.js`)
        ...
        .pipe(dest(paths.wordpress.js)) // put final JS in patternlab public folder
...
// Copy theme info from package to WordPress style.css
function themeInfoTask(){
    return src(`${paths.wordpress.css}style.css`)
    	.pipe(header('/*!\nTheme Name: ${pckg.themename}\nTheme URI: ${pckg.themeuri}\nAuthor: ${pckg.author}\nAuthor URI: ${pckg.authoruri}\nDescription: ${pckg.description}\nVersion: ${pckg.version}\n*/', {pckg : pckg}))
      .pipe(dest(paths.wordpress.css)) // put final CSS in wordpress theme folder
      .pipe(browserSync.stream());
}
...
function imageOptimizeTask() {
  return src(`${paths.src.images}*`)
    ...
    .pipe(dest(paths.wordpress.images));
}
